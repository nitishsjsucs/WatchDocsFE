...
export default function WatchPage() {
  ...
  // Load watch data and initialize Gemini chat (supports both preview and existing-watch modes)
  useEffect(() => {
    const loadWatch = async () => {
      ...
      if (previewUrl && !id) {
        const tempWatch: WatchItem = {
          ...
          url: previewUrl,
          // @ts-expect-error: preview is not part of the canonical status union in some projects
          status: 'preview',
...
What would be most valuable for you to know about when it changes on this site?`,
      ...
      // Existing watch mode - load from storage by ID
      if (id) {
...
What would be most valuable for you to know about when it changes on this site?`,
      ...
      // If neither ID nor preview URL is provided, go back to home
      navigate('/');
    };
  ...
  const handleCreateWatchDoc = () => {
    ...
    // Disable creating docs for preview mode (no persistence yet)
    const isPreview = String(watch.id) === 'preview' || (watch as any).status === 'preview';
    if (isPreview) {
      navigate('/');
      return;
    }

    // Persist notes / alert level to existing watch
    try {
      // Some projects use string IDs; ensure we pass the same shape back
      updateWatch(String(watch.id), {
              ...
              <div className="flex-1 flex flex-col p-4 sm:p-6">
                ...
                <div className="mt-6">
                  <Button
                    ...
                    size="lg"
                    disabled={isPreview}
                    title={isPreview ? 'Not available in preview mode' : 'Create Watch Doc'}
                  >
                    Create Watch Doc
                  </Button>
                </div>
